...
/*
 * Set up the first available rtl-sdr device to write into
 * the 'write_to' buffer 
 *
 */
void setup_rtl_device(int *write_to) {
   ...
   * process the data when it returns.
   * The processing function should be the same for any reader, 
   * in a separate thread for each.
   */
